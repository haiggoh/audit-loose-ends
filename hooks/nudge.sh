...
NUDGE="audit: when a substantive task ends, when the user signals to wrap up / conclude the session, or when THIS session has created or changed durable records (memories, project notes, reminders/crons, the task list, or the waypoints store) — run the audit reconciliation before closing. Scan those surfaces for anything REDUNDANT, ORPHANED, or FALSELY still-flagged as to-do when it's actually done, and fix it; distinguish a historical completion record (keep) from a stale pending flag on finished work (fix). Mark finished items done (e.g. \`waypoints done <id>\`), capture genuinely-open follow-ups as waypoints, and confirm touched repos are committed/clean with nothing accidentally public. This is NOT length-based — trigger on durable-state change, not on effort; skip read-only/exploratory turns. Consult the audit skill for the full procedure."
